"use client";

import * as React from "react";
import * as TooltipPrimitive from "@radix-ui/react-tooltip";
import { cn } from "@/lib/utils";
import { motion, AnimatePresence } from "framer-motion";

const TooltipProvider = TooltipPrimitive.Provider;

const Tooltip = TooltipPrimitive.Root;

const TooltipTrigger = TooltipPrimitive.Trigger;

const TooltipContent = React.forwardRef<
	React.ElementRef<typeof TooltipPrimitive.Content>,
	React.ComponentPropsWithoutRef<typeof TooltipPrimitive.Content>
>(({ className, sideOffset = 4, ...props }, ref) => (
	<TooltipPrimitive.Content
		ref={ref}
		sideOffset={sideOffset}
		className={cn(
			"z-50 overflow-hidden rounded-md bg-primary px-3 py-1.5 text-xs text-primary-foreground shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
			className
		)}
		{...props}
	/>
));
TooltipContent.displayName = TooltipPrimitive.Content.displayName;

const EnhancedTooltipProvider = ({ children, delayDuration = 300, skipDelayDuration = 300 }) => (
	<TooltipProvider delayDuration={delayDuration} skipDelayDuration={skipDelayDuration}>
		{children}
	</TooltipProvider>
);

const EnhancedTooltip = React.forwardRef<
	React.ElementRef<typeof TooltipPrimitive.Root>,
	React.ComponentPropsWithoutRef<typeof TooltipPrimitive.Root> & {
		content: React.ReactNode;
		side?: "top" | "right" | "bottom" | "left";
		align?: "start" | "center" | "end";
		dark?: boolean;
		wide?: boolean;
	}
>(({ children, content, side = "top", align = "center", dark = false, wide = false, ...props }, ref) => (
	<Tooltip {...props}>
		<TooltipTrigger asChild>{children}</TooltipTrigger>
		<AnimatePresence>
			<TooltipPrimitive.Content
				side={side}
				align={align}
				sideOffset={6}
				className={cn(
					"z-50 overflow-hidden rounded-md shadow-lg",
					"px-3 py-2",
					wide ? "min-w-[180px] max-w-[300px]" : "max-w-[200px]",
					dark
						? "bg-gray-900 text-gray-100"
						: "bg-white text-gray-800 border border-gray-200",
					"text-sm font-medium",
				)}
				asChild
			>
				<motion.div
					initial={{ opacity: 0, scale: 0.95 }}
					animate={{ opacity: 1, scale: 1 }}
					exit={{ opacity: 0, scale: 0.95 }}
					transition={{ duration: 0.15 }}
				>
					{content}
					<TooltipPrimitive.Arrow
						className={cn(
							"fill-current",
							dark ? "text-gray-900" : "text-white",
							"h-2 w-4"
						)}
					/>
				</motion.div>
			</TooltipPrimitive.Content>
		</AnimatePresence>
	</Tooltip>
));
EnhancedTooltip.displayName = "EnhancedTooltip";

export {
	Tooltip,
	TooltipTrigger,
	TooltipContent,
	TooltipProvider,
	EnhancedTooltip,
	EnhancedTooltipProvider,
};
